...
{
    /**
     * Flag di debug (cache per singola richiesta)
     *
     * @var bool|null
     */
    ...
    public function install()
    {
        ...
        return parent::install() &&
            ...
            Configuration::updateValue('PROMOTIONS_COUNTDOWN_DEBUG', 0);
    }
    ...
    public function uninstall()
    {
        ...
        return parent::uninstall() &&
            ...
            Configuration::deleteByName('PROMOTIONS_COUNTDOWN_DEBUG');
    }
    ...
    /**
     * Verifica se il modulo è in modalità debug (log verbosi / template diagnostici)
     */
    private function isDebugEnabled()
    {
        if ($this->debugEnabled === null) {
            $this->debugEnabled = (bool)Configuration::get('PROMOTIONS_COUNTDOWN_DEBUG');
        }
    ...
    public function hookDisplayProductPriceBlock($params)
    {
        try {
            ...
            if ($this->isDebugEnabled()) {
                ...
                $this->context->smarty->assign(['debug_info' => null]);
            }
            ...
            if ($is_countdown_best) {
                ...
                $price_info = $this->buildPricePresentation(
                    $product->id,
                    $id_product_attribute,
                    (float)$product_discount['discount_percent']
                );
                ...
                $this->context->smarty->assign([
                    ...
                    'original_price_formatted' => $price_info['original_formatted'],
                    'discounted_price_formatted' => $price_info['discounted_formatted'],
                ]);
    ...
    public function hookDisplayCheckoutSummary($params)
    {
        ...
        foreach ($cart->getProducts() as $cart_product) {
            ...
            $attribute_id = (int)$cart_product['id_product_attribute'];

            $best_discount = $this->getProductDiscount($product_id, $active_promotions);

            if (!$best_discount || !isset($best_discount['type']) || $best_discount['type'] !== 'countdown') {
                continue;
            }
            ...
            $price_info = $this->buildPricePresentation(
                $product_id,
                $attribute_id,
                (float)$best_discount['discount_percent']
            );
            ...
            $checkout_lines[] = [
                ...
                'original_price_formatted' => $price_info['original_formatted'],
                'discounted_price_formatted' => $price_info['discounted_formatted'],
                'discount_percent' => (float)$best_discount['discount_percent'],
            ];
        }

        if (empty($checkout_lines)) {
            return '';
        }

        $this->context->smarty->assign([
            'promotion_checkout_lines' => $checkout_lines,
        ]);
    ...
    private function getProductDiscount($product_id, $active_promotions)
    {
        ...
        $promotion_product_map = $this->getPromotionProductMap($active_promotions);
        if (!empty($active_promotions)) {
            foreach ($active_promotions as $promotion) {
                $promotion_id = (int)$promotion['id_promotion'];
                if (isset($promotion_product_map[$product_id]) && isset($promotion_product_map[$product_id][$promotion_id])) {
    ...
    /**
     * Calcola coppia prezzo originale / scontato per la percentuale passata
     */
    private function buildPricePresentation($product_id, $id_product_attribute, $discount_percent, $base_price = null)
    {
        $cart_id = $this->getContextCartId();
        $customer_id = (isset($this->context->customer) && $this->context->customer->id) ? (int)$this->context->customer->id : 0;
        ...
        $discounted_price = (float)$base_price * (1 - ($discount_percent / 100));

        return [
            'original' => (float)$base_price,
            'discounted' => $discounted_price,
            'original_formatted' => Tools::displayPrice($base_price),
            'discounted_formatted' => Tools::displayPrice($discounted_price),
        ];
    }
    ...
    /**
     * Ritorna una mappa prodotto => [id_promotion => true] per le promozioni passate
     */
    private function getPromotionProductMap(array $active_promotions)
    {
        static $cache = [];
        ...
        $promotion_ids = array_map(function ($promotion) {
            return (int)$promotion['id_promotion'];
        }, $active_promotions);
        ...
        sort($promotion_ids);
        $cache_key = implode('-', $promotion_ids);

        if (isset($cache[$cache_key])) {
            return $cache[$cache_key];
        }

        $sql = 'SELECT id_promotion, id_product FROM `'._DB_PREFIX_.'promotion_products` 
                WHERE id_promotion IN (' . implode(',', $promotion_ids) . ')';
        ...
        foreach ($rows as $row) {
            ...
            $promotion_id = (int)$row['id_promotion'];

            if (!isset($map[$product_id])) {
                $map[$product_id] = [];
            }

            $map[$product_id][$promotion_id] = true;
        }
